    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if(actionType === 'full_session') {
        ...
        const s = {
          ...
          userRef: refId || uid(),
          expiresAt: Date.now() + (20 * 60 * 1000) // 20 minutes from activation
        };
        DB.sessions.push(s);
        await saveDB();
      } else if(actionType === 'vip_initial') {
        // initial VIP payment ($10k) -> create VIP hold record
        const vip = {
          id: uid(),
          phase: 'initial_paid',
        ...
        // donation payment ($5k) -> find matching vip by ref or latest initial and mark complete
        ...
        if(vip){
          ...
          // create activated VIP session record
          const s = {
            id: uid(),
            type: 'vip_full',
            ...
            ref: vip.ref,
            expiresAt: Date.now() + (1000*60*60*2) // example long duration
          };
          DB.sessions.push(s);
          await saveDB();
        }
...
// AI / TTS via Gemini (expects GEMINI_API_KEY in env). This endpoint generates spoken audio file and returns url.
// Implementation assumes API returns base64 audio or binary. Adjust per actual Gemini API shape.
app.post('/ai-response', async (req, res) => {
  try {
    const { prompt, voice='Miguel', longForm=false } = req.body;
    // Build request to Gemini (user must set GEMINI_API_KEY in Render env)
    // This is a generic POST to an assumed Gemini TTS endpoint. Adapt headers/JSON as needed for the actual API.
    ...
    // Expect gJson.audioContent as base64
    ...
    // Save short conversation record
...
// Create VIP initial / donation checkout (two-step)
...
// Endpoint to get active sessions and DB (admin/debug)
